...
namespace PingPong.API.Features.ServerFeatures.AssignRoleRequest
{
    public sealed class AssignRole
    {
            ...
            : IRequestHandler<Command, Result>
        {
            public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    server.AssignRole(request.UserId, request.RoleId);
                }
        ...
        {
            group.MapPut("/{serverId:guid}/members/{userId:guid}/roles/{roleId:guid}", async (
                Guid serverId,
                Guid userId,
                Guid roleId,
                ISender sender,
